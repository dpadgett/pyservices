...
def mergematch(match, outdir):
  ...
  for demo in match['demos']:
    ...
    if longest_map is None or map['map_end_time'] - map['map_start_time'] > longest_map['map_end_time'] - longest_map['map_start_time']:
      longest_map = map
  ...
  for player in scores['redplayers']:
    description +=  str(player['time']) + ' ' + strip_html(strip_colors(player['client_name'])) + ' ' + str(player['score']) + ' ' + str(player['captures']) + "\r\n"
  description += "\r\nRendered from teh's JKA match database: http://dpadgett.org/jkademo/match.html#rpc=lookup&id=" + match['_id']
  video = dict(
    snippet = dict(
...
if __name__ == '__main__':
  ...
  dir = 'C:/Program Files (x86)/jka/base/demos/acur/test/'
  if len(sys.argv) > 2:
    dir = sys.argv[2]
  file = mergematch(match, dir)
